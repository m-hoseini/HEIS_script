...
R71P4S1$COL02 <- as.numeric(as.character(R71P4S1$COL02))
R71P4S1$COL03 <- as.numeric(as.character(R71P4S1$COL03))
R71P4S01 <- R71P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U71P4S1$COL02 <- as.numeric(as.character(U71P4S1$COL02))
U71P4S1$COL03 <- as.numeric(as.character(U71P4S1$COL03))
U71P4S01 <- U71P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 2
R71P4S2$COL02 <- as.numeric(as.character(R71P4S2$COL02))
R71P4S2$COL03 <- as.numeric(as.character(R71P4S2$COL03))
R71P4S02 <- R71P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U71P4S2$COL02 <- as.numeric(as.character(U71P4S2$COL02))
U71P4S2$COL03 <- as.numeric(as.character(U71P4S2$COL03))
U71P4S02 <- U71P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
rm(R71P4S1,U71P4S1,R71P4S2,U71P4S2,R71P4S3,U71P4S3)

rm(list = setdiff(ls(), ls(pattern = "71"))) # removing unnecessary objects

save.image(file="./exported/HEIS71.Rdata")
...
R71P3 <- bind_rows(mget(ls(pattern = "R71P3S.*")))
R71P3 <- R71P3 |>
  left_join(R71Data) |>
  mutate(urban = "R")

U71P3 <- bind_rows(mget(ls(pattern = "U71P3S.*")))
U71P3 <- U71P3 |>
  left_join(U71Data) |>
  mutate(urban = "U")
...
attr(EXP71$Table, "label") <- "Table in Part 3"
attr(EXP71$Global, "label") <- "global item code"
attr(EXP71$code, "label") <- "item code in this year"
attr(EXP71$urban, "label") <- "rural or urban"
attr(EXP71$Value, "label") <- "monthly expenditure"
attr(EXP71$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP71, "./exported/EXP71.Rds")


###############################################
# Building individual level data

r71data <- R71Data |> select(Address, weight, province)
u71data <- U71Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R71P4S01)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  'ISIC_w'= first(ISIC_w),
  ...
  by=.(Address, member)]
R71P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R71P4S02)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  'ISIC_s'= first(ISIC_s),
  ...
  by=.(Address, member)]
R71P4S02_unique <- as.data.frame(DT)

DT <- data.table(R71P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R71P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U71P4S01)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  'ISIC_w'= first(ISIC_w),
  'status_w'= first(status_w),
  ...
  'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
  by=.(Address, member)]
U71P4S01_unique <- as.data.frame(DT)

DT <- data.table(U71P4S02)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  'ISIC_s'= first(ISIC_s),
  'status_s'= first(status_s),
  ...
  'sale'= sum(sale, na.rm = T)),
  by=.(Address, member)]
U71P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U71P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            'income_resale'= sum(income_resale, na.rm = T)),
         by=.(Address, member)]
U71P4S03_unique <- as.data.frame(DT) 

Rind71 <- r71data  |>
  left_join(select(R71P1,-degree)) |>
  left_join(R71P4S01_unique) |>
  left_join(R71P4S02_unique) |>
  left_join(R71P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind71 <- u71data |>
  left_join(select(U71P1,-degree)) |>
  left_join(U71P4S01_unique) |>
  left_join(U71P4S02_unique) |>
  left_join(U71P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND71 <- bind_rows(Rind71,Uind71) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND71$occupationalst, "label") <- "employment status"
attr(IND71$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND71$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND71$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND71$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND71$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND71$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND71$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND71$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND71$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND71,"./exported/IND71.Rds")
...
attr(HH71$size, "label") <- "Household size"
attr(HH71$literates, "label") <- "Number of literate members"
attr(HH71$students, "label") <- "Number of student members"
attr(HH71$employeds, "label") <- "Number of employed members"
attr(HH71$gender, "label") <- "Head's gender"
attr(HH71$age, "label") <- "Head's age"
attr(HH71$literacy, "label") <- "Head's literacy"
attr(HH71$education, "label") <- "Head's education"
attr(HH71$occupationalst, "label") <- "Head's job status"
attr(HH71$maritalst, "label") <- "Head's marital status"
attr(HH71$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH71$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH71$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH71$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH71$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH71$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH71$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH71$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH71$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH71$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH71$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH71$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH71$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH71$income, "label") <- "total household income in previous year"
attr(HH71$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH71, file = "./exported/HH71.Rds")
...
df <- readRDS("./exported/HH71.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH71.dta") # to export in STATA
#write_csv2(df, "./exported/HH71.csv") # to export in CSV
