...
R95_weight <- Sum_R95 |> select(ADDRESS, weight) |> rename(Address = ADDRESS) |> mutate_if(is.character, as.numeric)
R95Data <- R95Data |> 
  ...
  left_join(R95_weight)

U95_weight <- Sum_U95 |> select(ADDRESS, weight) |> rename(Address = ADDRESS) |> mutate_if(is.character, as.numeric)
U95Data <- U95Data |> 
  rename(month = MahMorajeh, khanevartype = NoeKhn) |>
  mutate(province = fct_recode(as.factor(substr(Address, 2, 3)), !!!Province),
         town = as.integer(substr(Address, 4, 5)))%>%
  left_join(U95_weight)
...
education <- c(Elemantry="11", Secondary="21", HighSchool="31", Diploma="41", College="51", Bachelor="52", Master="53", PhD="61", Elemantry="71", NULL="", NULL="--",NULL="  ", NULL="1")
...
R95P1 <- R95P1 |> 
  ...
  mutate(across(where(is.character), as.integer),
         ...
         education = fct_recode(degree, !!!education), 
...
U95P3S06$DYCOL03<- as.numeric(levels(U95P3S06$DYCOL03))[U95P3S06$DYCOL03]
U95P3S06 <- U95P3S06 |>
  rename(
    code = DYCOL01,
    purchased = DYCOL02,
...
U95P4S01$DYCOL14<- as.numeric(levels(U95P4S01$DYCOL14))[U95P4S01$DYCOL14]
U95P4S01 <- U95P4S01 |>
  rename(
    member = DYCOL01,
    employed_w = DYCOL02,
...
rm(list = setdiff(ls(), ls(pattern = "95"))) # removing unnecessary objects

save.image(file="./exported/HEIS95.Rdata")
...
R95P3 <- bind_rows(mget(ls(pattern = "R95P3S.*")))
R95P3 <- R95P3 |> 
  left_join(R95Data) |>
  mutate(urban = "R")

U95P3 <- bind_rows(mget(ls(pattern = "U95P3S.*")))
U95P3 <- U95P3 |> 
  left_join(U95Data) |> 
  mutate(urban = "U")
...
attr(EXP95$Table, "label") <- "Table in Part 3"
attr(EXP95$Global, "label") <- "global item code"
attr(EXP95$code, "label") <- "item code in this year"
attr(EXP95$urban, "label") <- "rural or urban"
attr(EXP95$Price, "label") <- "median price"
attr(EXP95$Value, "label") <- "monthly expenditure"
attr(EXP95$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP95, "./exported/EXP95.Rds")

###############################################
# Building individual level data

r95data<- R95Data |> select(Address, month, weight, khanevartype, Jaygozin, province)
u95data<- U95Data |> select(Address, month, weight, khanevartype, Jaygozin, province)


# faster code with data.table
DT <- data.table(R95P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R95P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(R95P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R95P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R95P4S03)
...
DT <- DT[,.('employed_w'= first(employed_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U95P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(U95P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U95P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U95P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U95P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U95P4S04)
DT <- DT[,.('subsidy_number'= sum(subsidy_number, na.rm = T),
            'subsidy_month'= sum(subsidy_month, na.rm = T),
            'subsidy'= sum(subsidy, na.rm = T)),
         by=.(Address, member)]
U95P4S04_unique <- as.data.frame(DT) 

Rind95 <- r95data  |>
  left_join(select(R95P1,-degree)) |>
  left_join(R95P4S01_unique) |>
  left_join(R95P4S02_unique) |>
  left_join(R95P4S03_unique) |> 
  left_join(R95P4S04_unique) |>
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind95 <- u95data |>
  left_join(select(U95P1,-degree)) |>
  left_join(U95P4S01_unique) |>
  left_join(U95P4S02_unique) |>
  left_join(U95P4S03_unique) |> 
  left_join(U95P4S04_unique) |>
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND95 <- bind_rows(Rind95,Uind95) |>
  mutate(urban = as.factor(urban),
         Global = 1000000 ,
...
attr(IND95$occupationalst, "label") <- "employment status"
attr(IND95$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND95$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND95$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND95$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND95$hours_w, "label") <- "Wage-earning job: hours per day"
attr(IND95$days_w, "label") <- "Wage-earning job: day per week"
attr(IND95$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND95$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND95$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND95$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND95$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND95$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND95$hours_s, "label") <- "non-wage job: hours per day"
attr(IND95$days_s, "label") <- "non-wage job: day per week"
attr(IND95$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND95,"./exported/IND95.Rds")
...
attr(HH95$size, "label") <- "Household size"
attr(HH95$literates, "label") <- "Number of literate members"
attr(HH95$students, "label") <- "Number of student members"
attr(HH95$employeds, "label") <- "Number of employed members"
attr(HH95$gender, "label") <- "Head's gender"
attr(HH95$age, "label") <- "Head's age"
attr(HH95$literacy, "label") <- "Head's literacy"
attr(HH95$education, "label") <- "Head's education"
attr(HH95$occupationalst, "label") <- "Head's job status"
attr(HH95$maritalst, "label") <- "Head's marital status"
attr(HH95$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH95$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH95$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH95$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH95$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH95$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH95$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH95$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH95$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH95$income_nm_free, "label") <- "Non-monetary income from free goods/services"
attr(HH95$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH95$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(HH95$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH95$income, "label") <- "total household income in previous year"
attr(HH95$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH95, file = "./exported/HH95.Rds")
...
df <- readRDS("./exported/HH95.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH95.dta") # to export in STATA
#write_csv2(df, "./exported/HH95.csv") # to export in CSV
