...
R88P1 <- R88P1 |> 
  ...
  mutate(across(where(is.character), as.integer),
         ...
         studying = factor(studying, levels = c("1","2"), label=c("Yes","No")),
...
material <- c(MetalBlock="1", BrickWood="2", Cement="3", Brick="4", Wood="5", WoodKesht="6", KeshtGel="7", Other="8", NULL="0")
...
U88P3S03$DYCOL03 <- as.numeric(levels(U88P3S03$DYCOL03))[U88P3S03$DYCOL03]
U88P3S03 <- U88P3S03 |>
  rename(
    code = DYCOL01,
    purchased = DYCOL02,
...
# Part 3, Table 4
R88P3S04$DYCOL04 <- as.numeric(levels(R88P3S04$DYCOL04))[R88P3S04$DYCOL04]
R88P3S04 <- R88P3S04 |>
  rename(
    code = DYCOL01,
    mortgage = DYCOL02,
    purchased = DYCOL03,
    value = DYCOL04) |>
  mutate( code = ifelse(code %in% 45415:45419, 45413, code), #some wrong item codes
    table = 4L) |> 
  left_join(GlobalM) |>
  left_join(month) |>
  left_join(CPI) |>
...
R88P3S06$DYCOL03 <- as.numeric(levels(R88P3S06$DYCOL03))[R88P3S06$DYCOL03]
R88P3S06 <- R88P3S06 |>
  rename(
    code = DYCOL01,
    purchased = DYCOL02,
...
R88P3S09 <- R88P3S09 |>
  ...
  left_join(month) |>
  left_join(CPI) |>
  mutate(Global = ifelse(code==91211,2091211,Global), # a wrong item in this table
         value_r = value*100/cpi) |>
  select(Address:Global, value_r)
...
U88P3S12$DYCOL03 <- as.numeric(levels(U88P3S12$DYCOL03))[U88P3S12$DYCOL03]
U88P3S12 <- U88P3S12 |>
  rename(
    code = DYCOL01,
    purchased = DYCOL02,
...
R88P3S13 <- R88P3S13 |>
  rename(
    ...
    cost = DYCOL05,
    sell = DYCOL06) |>
  mutate( code = ifelse(code==72411, 72416,code),
...
U88P3S13 <- U88P3S13 |>
  ...
  left_join(month) |>
  left_join(CPI) |>
  mutate(Global = ifelse(code==73211,1073211,Global), # a wrong item in this table
         value_r=value*100/cpi_y) |>
  select(Address:Global, value_r)
...
U88P4S01$DYCOL03 <- as.numeric(levels(U88P4S01$DYCOL03))[U88P4S01$DYCOL03]
U88P4S01$DYCOL04 <- as.numeric(levels(U88P4S01$DYCOL04))[U88P4S01$DYCOL04]
...
R88P4S02$DYCOL03 <- as.numeric(levels(R88P4S02$DYCOL03))[R88P4S02$DYCOL03]
R88P4S02$DYCOL04 <- as.numeric(levels(R88P4S02$DYCOL04))[R88P4S02$DYCOL04]
R88P4S02$DYCOL15 <- as.numeric(levels(R88P4S02$DYCOL15))[R88P4S02$DYCOL15]
...
U88P4S02$DYCOL03 <- as.numeric(levels(U88P4S02$DYCOL03))[U88P4S02$DYCOL03]
U88P4S02$DYCOL04 <- as.numeric(levels(U88P4S02$DYCOL04))[U88P4S02$DYCOL04]
U88P4S02 <- U88P4S02 |>
  rename(
    member = DYCOL01,
    employed_s = DYCOL02,
...
rm(list = setdiff(ls(), ls(pattern = "88"))) # removing unnecessary objects

save.image(file="./exported/HEIS88.Rdata")
...
R88P3 <- bind_rows(mget(ls(pattern = "R88P3S.*")))
R88P3 <- R88P3 |>
  left_join(R88Data) |>
  mutate(urban = "R")

U88P3 <-  bind_rows(mget(ls(pattern = "U88P3S.*")))
U88P3 <- U88P3 |>
  left_join(U88Data) |>
  mutate(urban = "U")
...
attr(EXP88$Table, "label") <- "Table in Part 3"
attr(EXP88$Global, "label") <- "global item code"
attr(EXP88$code, "label") <- "item code in this year"
attr(EXP88$urban, "label") <- "rural or urban"
attr(EXP88$Price, "label") <- "median price"
attr(EXP88$Value, "label") <- "monthly expenditure"
attr(EXP88$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP88, "./exported/EXP88.Rds")

###############################################
# Building individual level data

r88data<- R88Data |> select(Address, month, weight, khanevartype, Jaygozin, province)
u88data<- U88Data |> select(Address, month, weight, khanevartype, Jaygozin, province)


# faster code with data.table
DT <- data.table(R88P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R88P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0, NA, .x)))

DT <- data.table(R88P4S02)
...
R88P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R88P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R88P4S03_unique <- as.data.frame(DT) 


DT <- data.table(U88P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U88P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0, NA, .x)))


DT <- data.table(U88P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U88P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U88P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U88P4S03_unique <- as.data.frame(DT) 


Rind88 <- r88data  |>
  left_join(select(R88P1,-degree)) |>
  left_join(R88P4S01_unique) |>
  left_join(R88P4S02_unique) |>
  left_join(R88P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind88 <- u88data |>
  left_join(select(U88P1,-degree)) |>
  left_join(U88P4S01_unique) |>
  left_join(U88P4S02_unique) |>
  left_join(U88P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND88 <- bind_rows(Rind88,Uind88)%>%
  mutate(urban = as.factor(urban),
         Global = 1000000 ,
...
attr(IND88$occupationalst, "label") <- "employment status"
attr(IND88$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND88$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND88$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND88$status_w, "label") <- "Wage-earning job status: 1-private 2-cooperative 3-public"
attr(IND88$hours_w, "label") <- "Wage-earning job: hours per day"
attr(IND88$days_w, "label") <- "Wage-earning job: day per week"
attr(IND88$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND88$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND88$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND88$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND88$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND88$status_s, "label") <- "job status: employer, selfemployed, familyworker"
attr(IND88$hours_s, "label") <- "non-wage job: hours per day"
attr(IND88$days_s, "label") <- "non-wage job: day per week"
attr(IND88$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND88,"./exported/IND88.Rds")
###############################################
...
attr(HH88$size, "label") <- "Household size"
attr(HH88$literates, "label") <- "Number of literate members"
attr(HH88$students, "label") <- "Number of student members"
attr(HH88$employeds, "label") <- "Number of employed members"
attr(HH88$gender, "label") <- "Head's gender"
attr(HH88$age, "label") <- "Head's age"
attr(HH88$literacy, "label") <- "Head's literacy"
attr(HH88$education, "label") <- "Head's education"
attr(HH88$occupationalst, "label") <- "Head's job status"
attr(HH88$maritalst, "label") <- "Head's marital status"
attr(HH88$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH88$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH88$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH88$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH88$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH88$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH88$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH88$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH88$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH88$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH88$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH88$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH88$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH88$income, "label") <- "total household income in previous year"
attr(HH88$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH88, file = "./exported/HH88.Rds")
...
df <- readRDS("./exported/HH88.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH88.dta") # to export in STATA
#write_csv2(df, "./exported/HH88.csv") # to export in CSV
