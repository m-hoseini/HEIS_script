...
R84Data <- Sum_R84 |> 
  select(Address = "ADDRESS", weight = "weight_R") |>
...
U84Data <- Sum_U84 |> 
  select(Address = "ADDRESS", weight = "weight_U") |>
  mutate(province = fct_recode(as.factor(substr(Address, 2, 3)), !!!Province),
         town = as.integer(substr(Address, 4, 5))) |>
  mutate_if(is.character, as.numeric)
...
R84P2 <- R84P2 |>
  rename(
    ...
    kitchen = DYCOL32,
    evapcooling = DYCOL33,
    centralcooling = DYCOL34,
    centralheating = DYCOL35,

    cookingfuel = DYCOL36,
    heatingfuel = DYCOL37,

    celebration_m = DYCOL38,
    celebration_y = DYCOL39,
    mourning_m = DYCOL40,
    mourning_y = DYCOL41,
    house_maintenance_m = DYCOL42,
    house_maintenance_y = DYCOL43,
    pilgrimage_m = DYCOL44,
    pilgrimage_y = DYCOL45,
    travel_abroad_m = DYCOL46,
    travel_abroad_y = DYCOL47,
    surgery_m = DYCOL48,
    #surgery_y = DYCOL56,
    occasions_other_m = DYCOL49,
    occasions_other_y = DYCOL50,
    none_2 = DYCOL51) |>
  mutate(across(where(is.character), as.integer),
         ...
         across(c(vehicle:centralheating,celebration_m:occasions_other_y), ~!is.na(.x)))
...
U84P3S01 <- U84p3S01 |>
  ...
  left_join(CPI) |>
  mutate(value_r=value*100/cpi) |>
  select(Address:Global, value_r, -DYCOL07)
...
R84P3S03 <- R84P3S03 |>
  rename(
    ...
    value = DYCOL03) |>
  mutate( 
    across(c(value),  ~ as.numeric(as.character(.x)) ),
    table = 3L) |> 
  left_join(GlobalM) |>
  left_join(CPI) |>
  mutate(value_r=value*100/cpi) |>
...
R84P3S09 <- R84P3S09 |>
  rename(
    ...
    purchased = DYCOL02,
    value = DYCOL03) |>
  mutate(code=ifelse(code==95121,95122,code), # this code is for Table 13
    across(c(value),  ~ as.numeric(as.character(.x)) ),
    table = 9L) |> 
  left_join(GlobalM) |>
  left_join(CPI) |>
...
R84P4S02$DYCOL15 <- as.numeric(as.character(R84P4S02$DYCOL15))
...
U84P4S02$DYCOL15 <- as.numeric(as.character(U84P4S02$DYCOL15))
...
U84P4S03$DYCOL06 <- as.numeric(as.character(U84P4S03$DYCOL06))
...
rm(list = setdiff(ls(), ls(pattern = "84"))) # removing unnecessary objects

save.image(file="./exported/HEIS84.Rdata")
...
R84P3 <- bind_rows(mget(ls(pattern = "R84P3S.*")))
R84P3 <- R84P3 |>
  left_join(R84Data) |>
  mutate(urban = "R")

U84P3 <- bind_rows(mget(ls(pattern = "U84P3S.*")))
U84P3 <- U84P3 |>
  left_join(U84Data) |>
  mutate(urban = "U")
...
attr(EXP84$Table, "label") <- "Table in Part 3"
attr(EXP84$Global, "label") <- "global item code"
attr(EXP84$code, "label") <- "item code in this year"
attr(EXP84$urban, "label") <- "rural or urban"
attr(EXP84$Price, "label") <- "median price"
attr(EXP84$Value, "label") <- "monthly expenditure"
attr(EXP84$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP84, "./exported/EXP84.Rds")

###############################################
# Building individual level data

r84data <- R84Data |> select(Address, weight, province)
u84data <- U84Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R84P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R84P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R84P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R84P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R84P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R84P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U84P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U84P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U84P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U84P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U84P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U84P4S03_unique <- as.data.frame(DT) 

Rind84 <- r84data  |>
  left_join(select(R84P1,-degree)) |>
  left_join(R84P4S01_unique) |>
  left_join(R84P4S02_unique) |>
  left_join(R84P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind84 <- u84data |>
  left_join(select(U84P1,-degree)) |>
  left_join(U84P4S01_unique) |>
  left_join(U84P4S02_unique) |>
  left_join(U84P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND84 <- bind_rows(Rind84,Uind84) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND84$occupationalst, "label") <- "employment status"
attr(IND84$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND84$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND84$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND84$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND84$hours_w, "label") <- "Wage-earning job: hours per day"
attr(IND84$days_w, "label") <- "Wage-earning job: day per week"
attr(IND84$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND84$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND84$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND84$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND84$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND84$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND84$hours_s, "label") <- "non-wage job: hours per day"
attr(IND84$days_s, "label") <- "non-wage job: day per week"
attr(IND84$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND84,"./exported/IND84.Rds")
...
attr(HH84$size, "label") <- "Household size"
attr(HH84$literates, "label") <- "Number of literate members"
attr(HH84$students, "label") <- "Number of student members"
attr(HH84$employeds, "label") <- "Number of employed members"
attr(HH84$gender, "label") <- "Head's gender"
attr(HH84$age, "label") <- "Head's age"
attr(HH84$literacy, "label") <- "Head's literacy"
attr(HH84$education, "label") <- "Head's education"
attr(HH84$occupationalst, "label") <- "Head's job status"
attr(HH84$maritalst, "label") <- "Head's marital status"
attr(HH84$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH84$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH84$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH84$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH84$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH84$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH84$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH84$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH84$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH84$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH84$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH84$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH84$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH84$income, "label") <- "total household income in previous year"
attr(HH84$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH84, file = "./exported/HH84.Rds")
...
df <- readRDS("./exported/HH84.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH84.dta") # to export in STATA
#write_csv2(df, "./exported/HH84.csv") # to export in CSV
