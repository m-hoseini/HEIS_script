...
R93_weight<-Sum_R93%>%select(ADDRESS, weight)%>%rename(Address = ADDRESS)%>%mutate_if(is.character, as.numeric)
R93Data <- R93Data %>% 
  ...
  left_join(R93_weight)


U93_weight<-Sum_U93%>%select(ADDRESS, weight)%>%rename(Address = ADDRESS)%>%mutate_if(is.character, as.numeric)
U93Data <- U93Data %>% 
  rename(month = MahMorajeh, khanevartype = NoeKhn) %>%
  mutate(province = fct_recode(as.factor(substr(Address, 2, 3)), !!!Province),
         town = as.integer(substr(Address, 4, 5)))%>%
  left_join(U93_weight)
...
# Part 3, Table 4
R93P3S04$DYCOL04 <- as.numeric(levels(R93P3S04$DYCOL04))[R93P3S04$DYCOL04]
R93P3S04 <- R93P3S04 %>%
  rename(
    code = DYCOL01,
    mortgage = DYCOL02,
...
rm(list = setdiff(ls(), ls(pattern = "93"))) # removing unnecessary objects

save.image(file="./exported/HEIS93.Rdata")
...
R93P3 <- bind_rows(mget(ls(pattern = "R93P3S.*")))
R93P3 <- R93P3 %>% 
  left_join(R93Data) %>%
  mutate(urban="R")

U93P3 <- bind_rows(mget(ls(pattern = "U93P3S.*")))
U93P3 <- U93P3 %>% 
  left_join(U93Data)%>%
  mutate(urban="U")
...
attr(EXP93$table, "label") <- "table number in Part 3"
attr(EXP93$gcode, "label") <- "global item code"
attr(EXP93$code, "label") <- "item code in this year"
attr(EXP93$urban, "label") <- "rural or urban"
attr(EXP93$Price, "label") <- "median price"
attr(EXP93$Value, "label") <- "monthly expenditure"

saveRDS(EXP93, "./exported/EXP93.Rds")

###############################################
# Building individual level data

r93data<- R93Data %>% select(Address, month, weight, khanevartype, Jaygozin, province)
u93data<- U93Data %>% select(Address, month, weight, khanevartype, Jaygozin, province)


# faster code with data.table
DT <- data.table(R93P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R93P4S01_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(R93P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R93P4S02_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R93P4S03)
...
DT <- DT[,.('employed_w'= first(employed_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U93P4S01_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(U93P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U93P4S02_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U93P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U93P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U93P4S04)
DT <- DT[,.('subsidy_number'= sum(subsidy_number, na.rm = T),
            'subsidy_month'= sum(subsidy_month, na.rm = T),
            'subsidy'= sum(subsidy, na.rm = T)),
         by=.(Address, member)]
U93P4S04_unique <- as.data.frame(DT) 


Rind93 <- r93data  %>%
  left_join(select(R93P1,-degree)) %>%
  left_join(R93P4S01_unique) %>%
  left_join(R93P4S02_unique) %>%
  left_join(R93P4S03_unique) %>% 
  left_join(R93P4S04_unique) %>%
  mutate(across(where(is.character),as.integer)) %>%
  mutate(urban = "R")

Uind93 <- u93data %>%
  left_join(select(U93P1,-degree)) %>%
  left_join(U93P4S01_unique) %>%
  left_join(U93P4S02_unique) %>%
  left_join(U93P4S03_unique) %>% 
  left_join(U93P4S04_unique) %>%
  mutate(across(where(is.character),as.integer)) %>%
  mutate(urban = "U")
Uind93$ISIC_w<- as.numeric(levels(Uind93$ISIC_w))[Uind93$ISIC_w]

IND93 <- bind_rows(Rind93,Uind93) %>%
  mutate(urban = as.factor(urban),
         DYCOL00 = NA_integer_,
...
attr(IND93$occupationalst, "label") <- "Job status"
attr(IND93$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND93$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND93$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND93$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND93$hours_w, "label") <- "Wage-earning job: hours per day"
attr(IND93$days_w, "label") <- "Wage-earning job: day per week"
attr(IND93$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND93$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND93$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND93$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND93$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND93$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND93$hours_s, "label") <- "non-wage job: hours per day"
attr(IND93$days_s, "label") <- "non-wage job: day per week"
attr(IND93$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND93,"./exported/IND93.Rds")
...
attr(HH93$size, "label") <- "Household size"
attr(HH93$literates, "label") <- "Number of literate members"
attr(HH93$students, "label") <- "Number of student members"
attr(HH93$employeds, "label") <- "Number of employed members"
attr(HH93$gender, "label") <- "Head's gender"
attr(HH93$age, "label") <- "Head's age"
attr(HH93$literacy, "label") <- "Head's literacy"
attr(HH93$education, "label") <- "Head's education"
attr(HH93$occupationalst, "label") <- "Head's job status"
attr(HH93$maritalst, "label") <- "Head's marital status"
attr(HH93$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH93$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH93$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH93$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH93$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH93$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH93$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH93$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH93$income_nm_miscellaneous, "label") <- "Non-monetary income from other sources (home production, free)"
attr(HH93$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH93$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH93$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH93$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH93$income, "label") <- "total household income in previous year"
attr(HH93$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH93, file = "./exported/HH93.Rds")
...
df <- readRDS("./exported/HH93.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH93.dta") # to export in STATA
#write_csv2(df, "./exported/HH93.csv") # to export in CSV
