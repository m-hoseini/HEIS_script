...
occupation <- c(employed="1", seasonalunemployed="2", IncomeWOJob="3", unemployed="4", Student="5", Housewife="6", unemployednotseeking="7", Other="8", Oldmember="9")
...
R63P1 <- R63P1 |> 
  rename(
    ...
    weeksinjob = COL15,
    nationality = COL08) |>
  mutate(literacy = ifelse(COL09 %in% c(1,2,3),1,2),
...
U63P1$COL01 <- as.numeric(as.character(U63P1$COL01))
U63P1 <- U63P1 |> 
  rename(
    Address = ADDRESS,
    member = COL01,
...
R63P4S1$COL02 <- as.numeric(as.character(R63P4S1$COL02))
R63P4S01 <- R63P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U63P4S1$COL01 <- as.numeric(as.character(U63P4S1$COL01))
U63P4S1$COL02 <- as.numeric(as.character(U63P4S1$COL02))
U63P4S01 <- U63P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 2
R63P4S2$COL02 <- as.numeric(as.character(R63P4S2$COL02))
R63P4S02 <- R63P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U63P4S2$COL02 <- as.numeric(as.character(U63P4S2$COL02))
U63P4S02 <- U63P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 3
R63P4S3$COL02 <- as.numeric(as.character(R63P4S3$COL02))
R63P4S03 <- R63P4S3 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U63P4S3$COL02 <- as.numeric(as.character(U63P4S3$COL02))
U63P4S03 <- U63P4S3 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 4
R63P4S4$COL02 <- as.numeric(as.character(R63P4S4$COL02))
R63P4S04 <- R63P4S4 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U63P4S4$COL02 <- as.numeric(as.character(U63P4S4$COL02))
U63P4S04 <- U63P4S4 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
rm(R63P4S1,U63P4S1,R63P4S2,U63P4S2,R63P4S3,U63P4S3)

rm(list = setdiff(ls(), ls(pattern = "63"))) # removing unnecessary objects

save.image(file="./exported/HEIS63.Rdata")
...
R63P3 <- bind_rows(mget(ls(pattern = "R63P3S.*")))
R63P3 <- R63P3 |>
  left_join(R63Data) |>
  mutate(urban = "R")

U63P3 <- bind_rows(mget(ls(pattern = "U63P3S.*")))
U63P3 <- U63P3 |>
  left_join(U63Data) |>
  mutate(urban = "U")
...
attr(EXP63$Table, "label") <- "Table in Part 3"
attr(EXP63$Global, "label") <- "global item code"
attr(EXP63$code, "label") <- "item code in this year"
attr(EXP63$urban, "label") <- "rural or urban"
attr(EXP63$Value, "label") <- "monthly expenditure"
attr(EXP63$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP63, "./exported/EXP63.Rds")
...
r63data <- R63Data |> select(Address, weight, province)
u63data <- U63Data |> select(Address, weight, province)

# faster code with data.table
R63P4S01_ <- bind_rows(R63P4S01,R63P4S02) |> arrange(Address,-netincome_w_y)
DT <- data.table(R63P4S01_)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  ...
  by=.(Address, member)]
R63P4S01_unique <- as.data.frame(DT) 

R63P4S02_ <- bind_rows(R63P4S03,R63P4S04) |> arrange(Address,-income_s_y)
DT <- data.table(R63P4S02_)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  ...
  by=.(Address, member)]
R63P4S02_unique <- as.data.frame(DT)

DT <- data.table(R63P4S05)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address)]
R63P4S03_unique <- as.data.frame(DT) 

U63P4S01_ <- bind_rows(U63P4S01,U63P4S02) |> arrange(Address,-netincome_w_y)
DT <- data.table(U63P4S01_)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  'status_w'= first(status_w),
  ...
  'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
  by=.(Address, member)]
U63P4S01_unique <- as.data.frame(DT) 

U63P4S02_ <- bind_rows(U63P4S03,U63P4S04) |> arrange(Address,-income_s_y)
DT <- data.table(U63P4S02_)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  'agriculture'= first(agriculture),
  'income_s_y'= sum(income_s_y, na.rm = T),
  ...
  'sale'= sum(sale, na.rm = T)),
  by=.(Address, member)]
U63P4S02_unique <- as.data.frame(DT)

DT <- data.table(U63P4S05)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            'income_resale'= sum(income_resale, na.rm = T)),
         by=.(Address)]
U63P4S03_unique <- as.data.frame(DT) 

Rind63 <- r63data  |>
  left_join(select(R63P1,-degree,-COL09,-occupational)) |>
  left_join(R63P4S01_unique) |>
  left_join(R63P4S02_unique) |>
  left_join(R63P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind63 <- u63data |>
  left_join(select(U63P1,-degree,-COL09,-occupational)) |>
  left_join(U63P4S01_unique) |>
  left_join(U63P4S02_unique) |>
  left_join(U63P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND63 <- bind_rows(Rind63,Uind63) |>
  mutate(urban = as.factor(urban),
...
attr(IND63$occupationalst, "label") <- "employment status"
attr(IND63$status, "label") <- "Overal job status"
attr(IND63$ISIC, "label") <- "Industry code of job"
attr(IND63$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND63$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND63$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND63$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND63$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND63,"./exported/IND63.Rds")
...
RHH63 <- r63data |> 
  mutate(urban = "R") |>
  left_join(r63p1, by="Address") |>
  left_join(r_head, by = "Address") |>
  left_join(r63p3, by = "Address") |>
...
attr(HH63$size, "label") <- "Household size"
attr(HH63$literates, "label") <- "Number of literate members"
attr(HH63$students, "label") <- "Number of student members"
attr(HH63$employeds, "label") <- "Number of employed members"
attr(HH63$gender, "label") <- "Head's gender"
attr(HH63$age, "label") <- "Head's age"
attr(HH63$literacy, "label") <- "Head's literacy"
attr(HH63$education, "label") <- "Head's education"
attr(HH63$occupationalst, "label") <- "Head's job status"
attr(HH63$maritalst, "label") <- "Head's marital status"
attr(HH63$ISIC, "label") <- "Head's Industry code"
attr(HH63$ISCO, "label") <- "Head's Occupation code"
attr(HH63$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH63$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH63$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH63$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH63$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH63$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH63$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH63$income, "label") <- "total household income in previous year"
attr(HH63$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH63, file = "./exported/HH63.Rds")
...
df <- readRDS("./exported/HH63.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH63.dta") # to export in STATA
#write_csv2(df, "./exported/HH63.csv") # to export in CSV
