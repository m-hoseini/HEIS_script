...
rm(R77P4S1,U77P4S1,R77P4S2,U77P4S2,R77P4S3,U77P4S3)

rm(list = setdiff(ls(), ls(pattern = "77"))) # removing unnecessary objects

save.image(file="./exported/HEIS77.Rdata")
...
R77P3 <- bind_rows(mget(ls(pattern = "R77P3S.*")))
R77P3 <- R77P3 |>
  left_join(R77Data) |>
  mutate(urban = "R") 

U77P3 <- bind_rows(mget(ls(pattern = "U77P3S.*")))
U77P3 <- U77P3 |>
  left_join(U77Data) |>
  mutate(urban = "U")
...
attr(EXP77$Table, "label") <- "table in Part 3"
attr(EXP77$Global, "label") <- "global item code"
attr(EXP77$code, "label") <- "item code in this year"
attr(EXP77$urban, "label") <- "rural or urban"
attr(EXP77$Price, "label") <- "median price"
attr(EXP77$Value, "label") <- "monthly expenditure"
attr(EXP77$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP77, "./exported/EXP77.Rds")

###############################################
# Building individual level data

r77data <- R77Data |> select(Address, weight, province)
u77data <- U77Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R77P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R77P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R77P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R77P4S02_unique <- as.data.frame(DT)

DT <- data.table(R77P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R77P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U77P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U77P4S01_unique <- as.data.frame(DT)

DT <- data.table(U77P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U77P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U77P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            'income_resale'= sum(income_resale, na.rm = T)),
         by=.(Address, member)]
U77P4S03_unique <- as.data.frame(DT) 

Rind77 <- r77data  |>
  left_join(select(R77P1,-degree)) |>
  left_join(R77P4S01_unique) |>
  left_join(R77P4S02_unique) |>
  left_join(R77P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind77 <- u77data |>
  left_join(select(U77P1,-degree)) |>
  left_join(U77P4S01_unique) |>
  left_join(U77P4S02_unique) |>
  left_join(U77P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND77 <- bind_rows(Rind77,Uind77) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND77$occupationalst, "label") <- "employment status"
attr(IND77$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND77$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND77$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND77$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND77$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND77$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND77$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND77$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND77$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND77$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND77$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND77,"./exported/IND77.Rds")
...
attr(HH77$size, "label") <- "Household size"
attr(HH77$literates, "label") <- "Number of literate members"
attr(HH77$students, "label") <- "Number of student members"
attr(HH77$employeds, "label") <- "Number of employed members"
attr(HH77$gender, "label") <- "Head's gender"
attr(HH77$age, "label") <- "Head's age"
attr(HH77$literacy, "label") <- "Head's literacy"
attr(HH77$education, "label") <- "Head's education"
attr(HH77$occupationalst, "label") <- "Head's job status"
attr(HH77$maritalst, "label") <- "Head's marital status"
attr(HH77$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH77$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH77$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH77$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH77$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH77$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH77$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH77$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH77$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH77$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH77$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH77$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH77$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH77$income, "label") <- "total household income in previous year"
attr(HH77$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH77, file = "./exported/HH77.Rds")
...
df <- readRDS("./exported/HH77.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH77.dta") # to export in STATA
#write_csv2(df, "./exported/HH77.csv") # to export in CSV
