...
rm(list = setdiff(ls(), ls(pattern = "96"))) # removing unnecessary objects

save.image(file="./exported/HEIS96.Rdata")
...
R96P3 <- bind_rows(mget(ls(pattern = "R96P3S.*")))
R96P3 <- R96P3 %>% 
  left_join(R96Data) %>%
  mutate(urban = "R")

U96P3 <- bind_rows(mget(ls(pattern = "U96P3S.*")))
U96P3 <- U96P3 %>% 
  left_join(U96Data) %>%
  mutate(urban = "U")
...
attr(EXP96$table, "label") <- "table number in Part 3"
attr(EXP96$gcode, "label") <- "global item code"
attr(EXP96$code, "label") <- "item code in this year"
attr(EXP96$urban, "label") <- "rural or urban"
attr(EXP96$Price, "label") <- "median price"
attr(EXP96$Value, "label") <- "monthly expenditure"
attr(EXP96$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP96, "./exported/EXP96.Rds")

###############################################
# Building individual level data


r96data<- R96Data %>% select(Address, month, weight, khanevartype, Jaygozin, province)
u96data<- U96Data %>% select(Address, month, weight, khanevartype, Jaygozin, province)


# faster code with data.table

DT <- data.table(R96P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R96P4S01_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(R96P4S02)
...
R96P4S02_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R96P4S03)
...
DT <- DT[,.('employed_w'= first(employed_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U96P4S01_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(U96P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
...
U96P4S02_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U96P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U96P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U96P4S04)
DT <- DT[,.('subsidy_number'= sum(subsidy_number, na.rm = T),
            'subsidy_month'= sum(subsidy_month, na.rm = T),
            'subsidy'= sum(subsidy, na.rm = T)),
         by=.(Address, member)]
U96P4S04_unique <- as.data.frame(DT) 


Rind96 <- r96data  %>%
  left_join(select(R96P1,-degree)) %>%
  left_join(R96P4S01_unique) %>%
  left_join(R96P4S02_unique) %>%
  left_join(R96P4S03_unique) %>% 
  left_join(R96P4S04_unique) %>%
  mutate(across(where(is.character),as.integer)) %>%
  mutate(urban = "R")

Uind96 <- u96data %>%
  left_join(select(U96P1,-degree)) %>%
  left_join(U96P4S01_unique) %>%
  left_join(U96P4S02_unique) %>%
  left_join(U96P4S03_unique) %>% 
  left_join(U96P4S04_unique) %>%
  mutate(across(where(is.character),as.integer)) %>%
  mutate(urban = "U")

IND96 <- bind_rows(Rind96,Uind96)%>%
  mutate(urban = as.factor(urban),
         DYCOL00 = NA_integer_ ,
...
attr(IND96$occupationalst, "label") <- "Job status"
attr(IND96$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND96$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND96$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND96$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND96$hours_w, "label") <- "Wage-earning job: hours per day"
attr(IND96$days_w, "label") <- "Wage-earning job: day per week"
attr(IND96$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND96$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND96$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND96$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND96$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND96$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND96$hours_s, "label") <- "non-wage job: hours per day"
attr(IND96$days_s, "label") <- "non-wage job: day per week"
attr(IND96$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND96,"./exported/IND96.Rds")
...
attr(HH96$size, "label") <- "Household size"
attr(HH96$literates, "label") <- "Number of literate members"
attr(HH96$students, "label") <- "Number of student members"
attr(HH96$employeds, "label") <- "Number of employed members"
attr(HH96$gender, "label") <- "Head's gender"
attr(HH96$age, "label") <- "Head's age"
attr(HH96$literacy, "label") <- "Head's literacy"
attr(HH96$education, "label") <- "Head's education"
attr(HH96$occupationalst, "label") <- "Head's job status"
attr(HH96$maritalst, "label") <- "Head's marital status"
attr(HH96$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH96$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH96$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH96$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH96$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH96$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH96$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH96$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH96$income_nm_miscellaneous, "label") <- "Non-monetary income from other sources (home production, free)"
attr(HH96$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH96$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH96$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH96$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH96$income, "label") <- "total household income in previous year"
attr(HH96$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH96, file = "./exported/HH96.Rds")
