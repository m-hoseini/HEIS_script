...
R70P4S1$COL02 <- as.numeric(as.character(R70P4S1$COL02))
R70P4S1$COL03 <- as.numeric(as.character(R70P4S1$COL03))
R70P4S01 <- R70P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U70P4S1$COL02 <- as.numeric(as.character(U70P4S1$COL02))
U70P4S1$COL03 <- as.numeric(as.character(U70P4S1$COL03))
U70P4S01 <- U70P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 2
R70P4S2$COL02 <- as.numeric(as.character(R70P4S2$COL02))
R70P4S2$COL03 <- as.numeric(as.character(R70P4S2$COL03))
R70P4S02 <- R70P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U70P4S2$COL02 <- as.numeric(as.character(U70P4S2$COL02))
U70P4S2$COL03 <- as.numeric(as.character(U70P4S2$COL03))
U70P4S02 <- U70P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
rm(R70P4S1,U70P4S1,R70P4S2,U70P4S2,R70P4S3,U70P4S3)

rm(list = setdiff(ls(), ls(pattern = "70"))) # removing unnecessary objects

save.image(file="./exported/HEIS70.Rdata")
...
R70P3 <- bind_rows(mget(ls(pattern = "R70P3S.*")))
R70P3 <- R70P3 |>
  left_join(R70Data) |>
  mutate(urban = "R")

U70P3 <- bind_rows(mget(ls(pattern = "U70P3S.*")))
U70P3 <- U70P3 |>
  left_join(U70Data) |>
  mutate(urban = "U")
...
attr(EXP70$Table, "label") <- "Table in Part 3"
attr(EXP70$Global, "label") <- "global item code"
attr(EXP70$code, "label") <- "item code in this year"
attr(EXP70$urban, "label") <- "rural or urban"
attr(EXP70$Value, "label") <- "monthly expenditure"
attr(EXP70$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP70, "./exported/EXP70.Rds")


###############################################
# Building individual level data

r70data <- R70Data |> select(Address, weight, province)
u70data <- U70Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R70P4S01)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  'ISIC_w'= first(ISIC_w),
  ...
  by=.(Address, member)]
R70P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R70P4S02)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  'ISIC_s'= first(ISIC_s),
  ...
  by=.(Address, member)]
R70P4S02_unique <- as.data.frame(DT)

DT <- data.table(R70P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R70P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U70P4S01)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  'ISIC_w'= first(ISIC_w),
  'status_w'= first(status_w),
  ...
  'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
  by=.(Address, member)]
U70P4S01_unique <- as.data.frame(DT)

DT <- data.table(U70P4S02)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  'ISIC_s'= first(ISIC_s),
  'status_s'= first(status_s),
  ...
  'sale'= sum(sale, na.rm = T)),
  by=.(Address, member)]
U70P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U70P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            'income_resale'= sum(income_resale, na.rm = T)),
         by=.(Address, member)]
U70P4S03_unique <- as.data.frame(DT) 

Rind70 <- r70data  |>
  left_join(select(R70P1,-degree)) |>
  left_join(R70P4S01_unique) |>
  left_join(R70P4S02_unique) |>
  left_join(R70P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind70 <- u70data |>
  left_join(select(U70P1,-degree)) |>
  left_join(U70P4S01_unique) |>
  left_join(U70P4S02_unique) |>
  left_join(U70P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND70 <- bind_rows(Rind70,Uind70) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND70$occupationalst, "label") <- "employment status"
attr(IND70$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND70$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND70$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND70$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND70$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND70$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND70$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND70$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND70$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND70,"./exported/IND70.Rds")
...
attr(HH70$size, "label") <- "Household size"
attr(HH70$literates, "label") <- "Number of literate members"
attr(HH70$students, "label") <- "Number of student members"
attr(HH70$employeds, "label") <- "Number of employed members"
attr(HH70$gender, "label") <- "Head's gender"
attr(HH70$age, "label") <- "Head's age"
attr(HH70$literacy, "label") <- "Head's literacy"
attr(HH70$education, "label") <- "Head's education"
attr(HH70$occupationalst, "label") <- "Head's job status"
attr(HH70$maritalst, "label") <- "Head's marital status"
attr(HH70$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH70$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH70$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH70$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH70$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH70$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH70$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH70$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH70$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH70$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH70$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH70$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH70$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH70$income, "label") <- "total household income in previous year"
attr(HH70$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH70, file = "./exported/HH70.Rds")
...
df <- readRDS("./exported/HH70.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH70.dta") # to export in STATA
#write_csv2(df, "./exported/HH70.csv") # to export in CSV
