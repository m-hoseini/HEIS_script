...
R89P1$DYCOL08 <- as.numeric(substr(sprintf("%03d", as.numeric(levels(R89P1$DYCOL08))[R89P1$DYCOL08]),1,1))
R89P1 <- R89P1 %>% 
  rename(
    member = DYCOL01,
    relation = DYCOL03,
...
U89P1$DYCOL08 <- as.numeric(substr(sprintf("%03d", as.numeric(levels(U89P1$DYCOL08))[U89P1$DYCOL08]),1,1))
U89P1 <- U89P1 %>% 
  rename(
    member = DYCOL01,
    relation = DYCOL03,
...
colnames(R89P3S04)<- c("Address", "DYCOL01", "DYCOL02", "DYCOL03", "DYCOL04")
colnames(U89P3S04)<- c("Address", "DYCOL01", "DYCOL02", "DYCOL03", "DYCOL04")
...
U89P3S04$DYCOL04 <- as.numeric(levels(U89P3S04$DYCOL04))[U89P3S04$DYCOL04]
...
R89P3S10 <- R89P3S10 %>%
  ...
  mutate(DYCOL00 = code)%>%mutate_if(is.character, as.numeric)
...
U89P3S12$DYCOL03 <- as.numeric(levels(U89P3S12$DYCOL03))[U89P3S12$DYCOL03]
...
colnames(R89P3S13)<- c("Address", "DYCOL01", "DYCOL02", "DYCOL03", "DYCOL04", "DYCOL05", "DYCOL06")
colnames(U89P3S13)<- c("Address", "DYCOL01", "DYCOL02", "DYCOL03", "DYCOL04", "DYCOL05", "DYCOL06")
...
R89P4S02$ISCO_s <- as.numeric(levels(R89P4S02$ISCO_s))[R89P4S02$ISCO_s]
R89P4S02$ISIC_s <- as.numeric(levels(R89P4S02$ISIC_s))[R89P4S02$ISIC_s]
...
rm(list = setdiff(ls(), ls(pattern = "89"))) # removing unnecessary objects

save.image(file="./exported/HEIS89.Rdata")
...
R89P3 <- bind_rows(mget(ls(pattern = "R89P3S.*")))
R89P3 <- R89P3 %>%
  left_join(R89Data) %>%
  mutate(urban = "R")

U89P3 <-  bind_rows(mget(ls(pattern = "U89P3S.*")))
U89P3 <- U89P3 %>%
  left_join(U89Data) %>%
  mutate(urban = "U")
...
attr(EXP89$table, "label") <- "table number in Part 3"
attr(EXP89$gcode, "label") <- "global item code"
attr(EXP89$code, "label") <- "item code in this year"
attr(EXP89$urban, "label") <- "rural or urban"
attr(EXP89$Price, "label") <- "median price"
attr(EXP89$Value, "label") <- "monthly expenditure"
attr(EXP89$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP89, "./exported/EXP89.Rds")

###############################################
# Building individual level data

r89data<- R89Data %>% select(Address, month, weight, khanevartype, Jaygozin, province)
u89data<- U89Data %>% select(Address, month, weight, khanevartype, Jaygozin, province)


# faster code with data.table
DT <- data.table(R89P4S01)
...
by=.(Address, member)]
R89P4S01_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R89P4S02)
...
R89P4S02_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R89P4S03)
...
by=.(Address, member)]
R89P4S03_unique <- as.data.frame(DT) 


DT <- data.table(U89P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
...
by=.(Address, member)]
U89P4S01_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(U89P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
...
by=.(Address, member)]
U89P4S02_unique <- as.data.frame(DT) %>%
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U89P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
...
by=.(Address, member)]
U89P4S03_unique <- as.data.frame(DT) 


Rind89 <- r89data  %>%
  left_join(R89P1) %>%
  left_join(R89P4S01_unique) %>%
  left_join(R89P4S02_unique) %>%
  left_join(R89P4S03_unique) %>% 
  mutate(across(where(is.character),as.integer)) %>%
  mutate(urban = "R")

Uind89 <- u89data %>%
  left_join(U89P1) %>%
  left_join(U89P4S01_unique) %>%
  left_join(U89P4S02_unique) %>%
  left_join(U89P4S03_unique) %>% 
  mutate(across(where(is.character),as.integer)) %>%
  mutate(urban = "U")

IND89 <- bind_rows(Rind89,Uind89)%>%
  mutate(urban = as.factor(urban),
         DYCOL00 = NA_integer_ ) %>%
  left_join(month) %>%
  left_join(CPI) %>%
...
attr(IND89$occupationalst, "label") <- "Job status"
attr(IND89$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND89$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND89$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND89$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND89$hours_w, "label") <- "Wage-earning job: hours per day"
attr(IND89$days_w, "label") <- "Wage-earning job: day per week"
attr(IND89$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND89$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND89$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND89$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND89$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND89$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND89$hours_s, "label") <- "non-wage job: hours per day"
attr(IND89$days_s, "label") <- "non-wage job: day per week"
attr(IND89$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND89,"./exported/IND89.Rds")
###############################################
...
HH89 <- bind_rows(RHH89, UHH89) %>%
  ...
  mutate(expenditure = cost_food + cost_tobacco + cost_clothing + cost_housing + cost_furniture + cost_health + cost_transport + cost_communication + cost_recreation +  cost_restaurant + cost_miscellaneous
         ...
         income = income_s_y + netincome_w_y + income_pension + income_rent + income_interest + income_aid + income_transfer + income_nm_agriculture + income_nm_miscellaneous + income_nm_public + income_nm_private + income_nm_nonagriculture + income_nm_house,
         DYCOL00 = NA_integer_ 
  ) %>%
  left_join(month) %>%
  left_join(CPI) %>%
...
attr(HH89$size, "label") <- "Household size"
attr(HH89$literates, "label") <- "Number of literate members"
attr(HH89$students, "label") <- "Number of student members"
attr(HH89$employeds, "label") <- "Number of employed members"
attr(HH89$gender, "label") <- "Head's gender"
attr(HH89$age, "label") <- "Head's age"
attr(HH89$literacy, "label") <- "Head's literacy"
attr(HH89$degree, "label") <- "Head's degree"
attr(HH89$occupationalst, "label") <- "Head's job status"
attr(HH89$maritalst, "label") <- "Head's marital status"
attr(HH89$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH89$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH89$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH89$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH89$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH89$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH89$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH89$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH89$income_nm_miscellaneous, "label") <- "Non-monetary income from other sources (home production, free)"
attr(HH89$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH89$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH89$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH89$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH89$income, "label") <- "total household income in previous year"
attr(HH89$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH89, file = "./exported/HH89.Rds")
...
df <- readRDS("./exported/HH89.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH89.dta") # to export in STATA
#write_csv2(df, "./exported/HH89.csv") # to export in CSV
