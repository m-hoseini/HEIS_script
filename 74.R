...
R74Data <- R74P1 |> 
  mutate(Address = str_pad(ADDRESS,8,pad="0"),
...
R74P4S1$ADDRESS <- as.numeric(as.character(R74P4S1$ADDRESS))
R74P4S1$COL03 <- as.numeric(as.character(R74P4S1$COL03))
R74P4S01 <- R74P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U74P4S1$ADDRESS <- as.numeric(as.character(U74P4S1$ADDRESS))
U74P4S1$COL03 <- as.numeric(as.character(U74P4S1$COL03))
U74P4S01 <- U74P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 2
R74P4S2$COL03 <- as.numeric(as.character(R74P4S2$COL03))
R74P4S02 <- R74P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U74P4S2$COL03 <- as.numeric(as.character(U74P4S2$COL03))
U74P4S02 <- U74P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
rm(R74P4S1,U74P4S1,R74P4S2,U74P4S2,R74P4S3,U74P4S3)

rm(list = setdiff(ls(), ls(pattern = "74"))) # removing unnecessary objects

save.image(file="./exported/HEIS74.Rdata")
...
R74P3 <- bind_rows(mget(ls(pattern = "R74P3S.*")))
R74P3 <- R74P3 |>
  left_join(R74Data) |>
  mutate(urban = "R")

U74P3 <- bind_rows(mget(ls(pattern = "U74P3S.*")))
U74P3 <- U74P3 |>
  left_join(U74Data) |>
  mutate(urban = "U")
...
attr(EXP74$Table, "label") <- "Table in Part 3"
attr(EXP74$Global, "label") <- "global item code"
attr(EXP74$code, "label") <- "item code in this year"
attr(EXP74$urban, "label") <- "rural or urban"
attr(EXP74$Price, "label") <- "median price"
attr(EXP74$Value, "label") <- "monthly expenditure"
attr(EXP74$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP74, "./exported/EXP74.Rds")

###############################################
# Building individual level data

r74data <- R74Data |> select(Address, weight, province)
u74data <- U74Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R74P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R74P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R74P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R74P4S02_unique <- as.data.frame(DT)

DT <- data.table(R74P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R74P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U74P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U74P4S01_unique <- as.data.frame(DT)

DT <- data.table(U74P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U74P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U74P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            'income_resale'= sum(income_resale, na.rm = T)),
         by=.(Address, member)]
U74P4S03_unique <- as.data.frame(DT) 

Rind74 <- r74data  |>
  left_join(select(R74P1,-degree)) |>
  left_join(R74P4S01_unique) |>
  left_join(R74P4S02_unique) |>
  left_join(R74P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind74 <- u74data |>
  left_join(select(U74P1,-degree)) |>
  left_join(U74P4S01_unique) |>
  left_join(U74P4S02_unique) |>
  left_join(U74P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND74 <- bind_rows(Rind74,Uind74) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND74$occupationalst, "label") <- "employment status"
attr(IND74$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND74$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND74$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND74$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND74$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND74$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND74$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND74$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND74$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND74$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND74$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND74,"./exported/IND74.Rds")
...
attr(HH74$size, "label") <- "Household size"
attr(HH74$literates, "label") <- "Number of literate members"
attr(HH74$students, "label") <- "Number of student members"
attr(HH74$employeds, "label") <- "Number of employed members"
attr(HH74$gender, "label") <- "Head's gender"
attr(HH74$age, "label") <- "Head's age"
attr(HH74$literacy, "label") <- "Head's literacy"
attr(HH74$education, "label") <- "Head's education"
attr(HH74$occupationalst, "label") <- "Head's job status"
attr(HH74$maritalst, "label") <- "Head's marital status"
attr(HH74$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH74$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH74$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH74$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH74$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH74$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH74$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH74$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH74$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH74$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH74$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH74$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH74$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH74$income, "label") <- "total household income in previous year"
attr(HH74$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH74, file = "./exported/HH74.Rds")
...
df <- readRDS("./exported/HH74.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH74.dta") # to export in STATA
#write_csv2(df, "./exported/HH74.csv") # to export in CSV
