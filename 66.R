...
R66P4S1$COL02 <- as.numeric(as.character(R66P4S1$COL02))
R66P4S01 <- R66P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U66P4S1$COL02 <- as.numeric(as.character(U66P4S1$COL02))
U66P4S01 <- U66P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 2
R66P4S2$COL02 <- as.numeric(as.character(R66P4S2$COL02))
R66P4S02 <- R66P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U66P4S2$COL02 <- as.numeric(as.character(U66P4S2$COL02))
U66P4S02 <- U66P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 3
R66P4S3$COL01 <- as.numeric(as.character(R66P4S3$COL01))
R66P4S3$COL02 <- as.numeric(as.character(R66P4S3$COL02))
R66P4S03 <- R66P4S3 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U66P4S3$COL02 <- as.numeric(as.character(U66P4S3$COL02))
U66P4S03 <- U66P4S3 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 4
R66P4S4$COL02 <- as.numeric(as.character(R66P4S4$COL02))
R66P4S04 <- R66P4S4 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U66P4S4$COL02 <- as.numeric(as.character(U66P4S4$COL02))
U66P4S04 <- U66P4S4 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
rm(R66P4S1,U66P4S1,R66P4S2,U66P4S2,R66P4S3,U66P4S3)

rm(list = setdiff(ls(), ls(pattern = "66"))) # removing unnecessary objects

save.image(file="./exported/HEIS66.Rdata")
...
R66P3 <- bind_rows(mget(ls(pattern = "R66P3S.*")))
R66P3 <- R66P3 |>
  left_join(R66Data) |>
  mutate(urban = "R")

U66P3 <- bind_rows(mget(ls(pattern = "U66P3S.*")))
U66P3 <- U66P3 |>
  left_join(U66Data) |>
  mutate(urban = "U")
...
attr(EXP66$Table, "label") <- "Table in Part 3"
attr(EXP66$Global, "label") <- "global item code"
attr(EXP66$code, "label") <- "item code in this year"
attr(EXP66$urban, "label") <- "rural or urban"
attr(EXP66$Value, "label") <- "monthly expenditure"
attr(EXP66$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP66, "./exported/EXP66.Rds")


###############################################
# Building individual level data

r66data <- R66Data |> select(Address, weight, province)
u66data <- U66Data |> select(Address, weight, province)

# faster code with data.table
R66P4S01_ <- bind_rows(R66P4S01,R66P4S02) |> arrange(Address,-netincome_w_y)
DT <- data.table(R66P4S01_)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  ...
  by=.(Address, member)]
R66P4S01_unique <- as.data.frame(DT) 

R66P4S02_ <- bind_rows(R66P4S03,R66P4S04) |> arrange(Address,-income_s_y)
DT <- data.table(R66P4S02_)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  ...
  by=.(Address, member)]
R66P4S02_unique <- as.data.frame(DT)

DT <- data.table(R66P4S05)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address)]
R66P4S03_unique <- as.data.frame(DT) 

U66P4S01_ <- bind_rows(U66P4S01,U66P4S02) |> arrange(Address,-netincome_w_y)
DT <- data.table(U66P4S01_)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  'status_w'= first(status_w),
  ...
  'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
  by=.(Address, member)]
U66P4S01_unique <- as.data.frame(DT) 

U66P4S02_ <- bind_rows(U66P4S03,U66P4S04) |> arrange(Address,-income_s_y)
DT <- data.table(U66P4S02_)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  'agriculture'= first(agriculture),
  'income_s_y'= sum(income_s_y, na.rm = T),
  ...
  'sale'= sum(sale, na.rm = T)),
  by=.(Address, member)]
U66P4S02_unique <- as.data.frame(DT)

DT <- data.table(U66P4S05)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            'income_resale'= sum(income_resale, na.rm = T)),
         by=.(Address)]
U66P4S03_unique <- as.data.frame(DT) 

Rind66 <- r66data  |>
  left_join(select(R66P1,-degree,-COL07,-occupational)) |>
  left_join(R66P4S01_unique) |>
  left_join(R66P4S02_unique) |>
  left_join(R66P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind66 <- u66data |>
  left_join(select(U66P1,-degree,-COL07,-occupational)) |>
  left_join(U66P4S01_unique) |>
  left_join(U66P4S02_unique) |>
  left_join(U66P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND66 <- bind_rows(Rind66,Uind66) |>
  mutate(urban = as.factor(urban),
...
attr(IND66$occupationalst, "label") <- "employment status"
attr(IND66$status, "label") <- "Overal job status"
attr(IND66$ISIC, "label") <- "Industry code of job"
attr(IND66$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND66$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND66$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND66$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND66$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND66,"./exported/IND66.Rds")
...
RHH66 <- r66data |> 
  mutate(urban = "R") |>
  left_join(r66p1, by="Address") |>
  left_join(r_head, by = "Address") |>
  left_join(r66p3, by = "Address") |>
...
attr(HH66$size, "label") <- "Household size"
attr(HH66$literates, "label") <- "Number of literate members"
attr(HH66$students, "label") <- "Number of student members"
attr(HH66$employeds, "label") <- "Number of employed members"
attr(HH66$gender, "label") <- "Head's gender"
attr(HH66$age, "label") <- "Head's age"
attr(HH66$literacy, "label") <- "Head's literacy"
attr(HH66$education, "label") <- "Head's education"
attr(HH66$occupationalst, "label") <- "Head's job status"
#attr(HH66$maritalst, "label") <- "Head's marital status"
attr(HH66$ISIC, "label") <- "Head's Industry code"
attr(HH66$ISCO, "label") <- "Head's Occupation code"
attr(HH66$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH66$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH66$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH66$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH66$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH66$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH66$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH66$income, "label") <- "total household income in previous year"
attr(HH66$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH66, file = "./exported/HH66.Rds")
...
df <- readRDS("./exported/HH66.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH66.dta") # to export in STATA
#write_csv2(df, "./exported/HH66.csv") # to export in CSV
