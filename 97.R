...
U97Data <- U97Data |> 
  ...
  mutate(province = fct_recode(as.factor(substr(Address, 2, 3)), !!!Province),
         town = as.integer(substr(Address, 4, 5)),
         month = ifelse(month==0,6,month)) # for one urban household month is recorded 0
...
rm(list = setdiff(ls(), ls(pattern = "97"))) # removing unnecessary objects

save.image(file="./exported/HEIS97.Rdata")
...
R97P3 <- bind_rows(mget(ls(pattern = "R97P3S.*")))
R97P3 <- R97P3 |> 
  left_join(R97Data) |>
  mutate(urban = "R")

U97P3 <- bind_rows(mget(ls(pattern = "U97P3S.*")))
U97P3 <- U97P3 |> 
  left_join(U97Data) |> 
  mutate(urban = "U")
...
attr(EXP97$Table, "label") <- "Table in Part 3"
attr(EXP97$Global, "label") <- "global item code"
attr(EXP97$code, "label") <- "item code in this year"
attr(EXP97$urban, "label") <- "rural or urban"
attr(EXP97$Price, "label") <- "median price"
attr(EXP97$Value, "label") <- "monthly expenditure"
attr(EXP97$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP97, "./exported/EXP97.Rds")

###############################################
# Building individual level data

r97data<- R97Data |> select(Address, month, weight, khanevartype, Jaygozin, province)
u97data<- U97Data |> select(Address, month, weight, khanevartype, Jaygozin, province)


# Multi-job people are repeated and need to be unified. 
# faster code with data.table
DT <- data.table(R97P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R97P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(R97P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R97P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R97P4S03)
...
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
U97P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(U97P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
         ...
         by=.(Address, member)]
U97P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U97P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
         ...
         by=.(Address, member)]
U97P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U97P4S04)
DT <- DT[,.('subsidy_number'= sum(subsidy_number, na.rm = T),
            'subsidy_month'= sum(subsidy_month, na.rm = T),
            'subsidy'= sum(subsidy, na.rm = T)
            ),
         by=.(Address, member)]
U97P4S04_unique <- as.data.frame(DT) 

Rind97 <- r97data  |>
  left_join(R97P1) |>
  left_join(R97P4S01_unique) |>
  left_join(R97P4S02_unique) |>
  left_join(R97P4S03_unique) |> 
  left_join(R97P4S04_unique) |>
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind97 <- u97data |>
  left_join(U97P1) |>
  left_join(U97P4S01_unique) |>
  left_join(U97P4S02_unique) |>
  left_join(U97P4S03_unique) |> 
  left_join(U97P4S04_unique) |>
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND97 <- bind_rows(Rind97,Uind97) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND97$occupationalst, "label") <- "employment status"
attr(IND97$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND97$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND97$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND97$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND97$hours_w, "label") <- "Wage-earning job: hours per day"
attr(IND97$days_w, "label") <- "Wage-earning job: day per week"
attr(IND97$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND97$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND97$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND97$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND97$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND97$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND97$hours_s, "label") <- "non-wage job: hours per day"
attr(IND97$days_s, "label") <- "non-wage job: day per week"
attr(IND97$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND97,"./exported/IND97.Rds")
...
attr(HH97$size, "label") <- "Household size"
attr(HH97$literates, "label") <- "Number of literate members"
attr(HH97$students, "label") <- "Number of student members"
attr(HH97$employeds, "label") <- "Number of employed members"
attr(HH97$gender, "label") <- "Head's gender"
attr(HH97$age, "label") <- "Head's age"
attr(HH97$literacy, "label") <- "Head's literacy"
attr(HH97$education, "label") <- "Head's education"
attr(HH97$occupationalst, "label") <- "Head's job status"
attr(HH97$maritalst, "label") <- "Head's marital status"
attr(HH97$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH97$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH97$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH97$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH97$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH97$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH97$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH97$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH97$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH97$income_nm_free, "label") <- "Non-monetary income from free goods/services"
attr(HH97$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH97$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(HH97$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH97$income, "label") <- "total household income in previous year"
attr(HH97$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH97, file = "./exported/HH97.Rds")
...
df <- readRDS("./exported/HH97.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH97.dta") # to export in STATA
#write_csv2(df, "./exported/HH97.csv") # to export in CSV
