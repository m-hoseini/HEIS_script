...
rm(R81P4S1,U81P4S1,R81P4S2,U81P4S2,R81P4S3,U81P4S3)

rm(list = setdiff(ls(), ls(pattern = "81"))) # removing unnecessary objects

save.image(file="./exported/HEIS81.Rdata")
...
R81P3 <- bind_rows(mget(ls(pattern = "R81P3S.*")))
R81P3 <- R81P3 |>
  left_join(R81Data) |>
  mutate(urban = "R") 

U81P3 <- bind_rows(mget(ls(pattern = "U81P3S.*")))
U81P3 <- U81P3 |>
  left_join(U81Data) |>
  mutate(urban = "U")
...
attr(EXP81$Table, "label") <- "Table in Part 3"
attr(EXP81$Global, "label") <- "global item code"
attr(EXP81$code, "label") <- "item code in this year"
attr(EXP81$urban, "label") <- "rural or urban"
attr(EXP81$Price, "label") <- "median price"
attr(EXP81$Value, "label") <- "monthly expenditure"
attr(EXP81$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP81, "./exported/EXP81.Rds")

###############################################
# Building individual level data

r81data <- R81Data |> select(Address, weight, province)
u81data <- U81Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R81P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R81P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R81P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R81P4S02_unique <- as.data.frame(DT)

DT <- data.table(R81P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R81P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U81P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U81P4S01_unique <- as.data.frame(DT)

DT <- data.table(U81P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U81P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U81P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U81P4S03_unique <- as.data.frame(DT) 

Rind81 <- r81data  |>
  left_join(select(R81P1,-degree)) |>
  left_join(R81P4S01_unique) |>
  left_join(R81P4S02_unique) |>
  left_join(R81P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind81 <- u81data |>
  left_join(select(U81P1,-degree)) |>
  left_join(U81P4S01_unique) |>
  left_join(U81P4S02_unique) |>
  left_join(U81P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND81 <- bind_rows(Rind81,Uind81) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND81$occupationalst, "label") <- "employment status"
attr(IND81$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND81$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND81$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND81$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND81$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND81$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND81$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND81$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND81$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND81$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND81$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND81,"./exported/IND81.Rds")
...
attr(HH81$size, "label") <- "Household size"
attr(HH81$literates, "label") <- "Number of literate members"
attr(HH81$students, "label") <- "Number of student members"
attr(HH81$employeds, "label") <- "Number of employed members"
attr(HH81$gender, "label") <- "Head's gender"
attr(HH81$age, "label") <- "Head's age"
attr(HH81$literacy, "label") <- "Head's literacy"
attr(HH81$education, "label") <- "Head's education"
attr(HH81$occupationalst, "label") <- "Head's job status"
attr(HH81$maritalst, "label") <- "Head's marital status"
attr(HH81$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH81$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH81$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH81$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH81$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH81$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH81$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH81$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH81$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH81$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH81$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH81$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH81$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH81$income, "label") <- "total household income in previous year"
attr(HH81$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH81, file = "./exported/HH81.Rds")
...
df <- readRDS("./exported/HH81.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH81.dta") # to export in STATA
#write_csv2(df, "./exported/HH81.csv") # to export in CSV
