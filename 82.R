...
R82P2 <- R82P2 |>
  rename(
    ...
    kitchen = Q6_08,
    evapcooling = Q6_05,
    centralcooling = Q6_09,
    centralheating = Q6_07,
    cookingfuel = Q7A,
    heatingfuel = Q7B) |>
  mutate(across(where(is.character), as.integer),
...
rm(R82P4S1,U82P4S1,R82P4S2,U82P4S2,R82P4S3,U82P4S3)

rm(list = setdiff(ls(), ls(pattern = "82"))) # removing unnecessary objects

save.image(file="./exported/HEIS82.Rdata")
...
R82P3 <- bind_rows(mget(ls(pattern = "R82P3S.*")))
R82P3 <- R82P3 |>
  left_join(R82Data) |>
  mutate(urban = "R") 

U82P3 <- bind_rows(mget(ls(pattern = "U82P3S.*")))
U82P3 <- U82P3 |>
  left_join(U82Data) |>
  mutate(urban = "U")
...
attr(EXP82$Table, "label") <- "Table in Part 3"
attr(EXP82$Global, "label") <- "global item code"
attr(EXP82$code, "label") <- "item code in this year"
attr(EXP82$urban, "label") <- "rural or urban"
attr(EXP82$Price, "label") <- "median price"
attr(EXP82$Value, "label") <- "monthly expenditure"
attr(EXP82$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP82, "./exported/EXP82.Rds")

###############################################
# Building individual level data

r82data <- R82Data |> select(Address, weight, province)
u82data <- U82Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R82P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R82P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R82P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R82P4S02_unique <- as.data.frame(DT)

DT <- data.table(R82P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R82P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U82P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U82P4S01_unique <- as.data.frame(DT)

DT <- data.table(U82P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U82P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U82P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U82P4S03_unique <- as.data.frame(DT) 

Rind82 <- r82data  |>
  left_join(select(R82P1,-degree)) |>
  left_join(R82P4S01_unique) |>
  left_join(R82P4S02_unique) |>
  left_join(R82P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind82 <- u82data |>
  left_join(select(U82P1,-degree)) |>
  left_join(U82P4S01_unique) |>
  left_join(U82P4S02_unique) |>
  left_join(U82P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND82 <- bind_rows(Rind82,Uind82) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND82$occupationalst, "label") <- "employment status"
attr(IND82$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND82$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND82$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND82$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND82$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND82$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND82$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND82$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND82$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND82$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND82$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND82,"./exported/IND82.Rds")
...
attr(HH82$size, "label") <- "Household size"
attr(HH82$literates, "label") <- "Number of literate members"
attr(HH82$students, "label") <- "Number of student members"
attr(HH82$employeds, "label") <- "Number of employed members"
attr(HH82$gender, "label") <- "Head's gender"
attr(HH82$age, "label") <- "Head's age"
attr(HH82$literacy, "label") <- "Head's literacy"
attr(HH82$education, "label") <- "Head's education"
attr(HH82$occupationalst, "label") <- "Head's job status"
attr(HH82$maritalst, "label") <- "Head's marital status"
attr(HH82$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH82$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH82$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH82$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH82$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH82$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH82$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH82$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH82$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH82$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH82$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH82$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH82$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH82$income, "label") <- "total household income in previous year"
attr(HH82$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH82, file = "./exported/HH82.Rds")
...
df <- readRDS("./exported/HH82.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH82.dta") # to export in STATA
#write_csv2(df, "./exported/HH82.csv") # to export in CSV
