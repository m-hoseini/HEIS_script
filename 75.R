...
rm(R75P4S1,U75P4S1,R75P4S2,U75P4S2,R75P4S3,U75P4S3)

rm(list = setdiff(ls(), ls(pattern = "75"))) # removing unnecessary objects

save.image(file="./exported/HEIS75.Rdata")
...
R75P3 <- bind_rows(mget(ls(pattern = "R75P3S.*")))
R75P3 <- R75P3 |>
  left_join(R75Data) |>
  mutate(urban = "R")

U75P3 <- bind_rows(mget(ls(pattern = "U75P3S.*")))
U75P3 <- U75P3 |>
  left_join(U75Data) |>
  mutate(urban = "U")
...
attr(EXP75$Table, "label") <- "Table in Part 3"
attr(EXP75$Global, "label") <- "global item code"
attr(EXP75$code, "label") <- "item code in this year"
attr(EXP75$urban, "label") <- "rural or urban"
attr(EXP75$Price, "label") <- "median price"
attr(EXP75$Value, "label") <- "monthly expenditure"
attr(EXP75$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP75, "./exported/EXP75.Rds")

###############################################
# Building individual level data

r75data <- R75Data |> select(Address, weight, province)
u75data <- U75Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R75P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R75P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R75P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R75P4S02_unique <- as.data.frame(DT)

DT <- data.table(R75P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R75P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U75P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U75P4S01_unique <- as.data.frame(DT)

DT <- data.table(U75P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U75P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U75P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            'income_resale'= sum(income_resale, na.rm = T)),
         by=.(Address, member)]
U75P4S03_unique <- as.data.frame(DT) 

Rind75 <- r75data  |>
  left_join(select(R75P1,-degree)) |>
  left_join(R75P4S01_unique) |>
  left_join(R75P4S02_unique) |>
  left_join(R75P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind75 <- u75data |>
  left_join(select(U75P1,-degree)) |>
  left_join(U75P4S01_unique) |>
  left_join(U75P4S02_unique) |>
  left_join(U75P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND75 <- bind_rows(Rind75,Uind75) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND75$occupationalst, "label") <- "employment status"
attr(IND75$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND75$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND75$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND75$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND75$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND75$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND75$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND75$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND75$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND75$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND75$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND75,"./exported/IND75.Rds")
...
attr(HH75$size, "label") <- "Household size"
attr(HH75$literates, "label") <- "Number of literate members"
attr(HH75$students, "label") <- "Number of student members"
attr(HH75$employeds, "label") <- "Number of employed members"
attr(HH75$gender, "label") <- "Head's gender"
attr(HH75$age, "label") <- "Head's age"
attr(HH75$literacy, "label") <- "Head's literacy"
attr(HH75$education, "label") <- "Head's education"
attr(HH75$occupationalst, "label") <- "Head's job status"
attr(HH75$maritalst, "label") <- "Head's marital status"
attr(HH75$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH75$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH75$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH75$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH75$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH75$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH75$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH75$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH75$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH75$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH75$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH75$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH75$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH75$income, "label") <- "total household income in previous year"
attr(HH75$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH75, file = "./exported/HH75.Rds")
...
df <- readRDS("./exported/HH75.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH75.dta") # to export in STATA
#write_csv2(df, "./exported/HH75.csv") # to export in CSV
