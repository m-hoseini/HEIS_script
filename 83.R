...
R83Data <- Sum_R83 |> 
  select(Address, weight = weight_R) |>
...
R83P2 <- R83P2 |>
  rename(
    ...
    stove = DYCOL18,
    vacuum = DYCOL19,
    washingmachine = DYCOL20,
    sewingmachine = DYCOL21,
    fan = DYCOL22,
    evapcoolingportable = DYCOL23,
    none = DYCOL24,
    pipewater = DYCOL25,
    electricity = DYCOL26,
    pipegas = DYCOL27,
    ...
    kitchen = DYCOL31,
    evapcooling = DYCOL32,
    centralcooling = DYCOL33,
    centralheating = DYCOL34,
    cookingfuel = DYCOL35,
    heatingfuel = DYCOL36) |>
  mutate(across(where(is.character), as.integer),
         ...
         across(c(vehicle:centralheating), ~!is.na(.x)))
...
R83P3S09 <- R83P3S09 |>
  rename(
    ...
    purchased = COL3,
    value = COL4) |>
  mutate(code=ifelse(code==95121,95122,code), 
    table = 9L) |> 
  left_join(GlobalM) |>
  left_join(CPI) |>
  mutate(value_r=value*100/cpi) |>
...
R83P3S13 <- R83P3S13 |>
  rename(
    ...
    cost = COL4,
    sell = COL5) |>
  mutate(code=ifelse(code %in% c(53126,53127),33418,code), # Water heater goes to Table 14 after 1383
...
rm(R83P4S1,U83P4S1,R83P4S2,U83P4S2,R83P4S3,U83P4S3)


rm(list = setdiff(ls(), ls(pattern = "83"))) # removing unnecessary objects

save.image(file="./exported/HEIS83.Rdata")
...
R83P3 <- bind_rows(mget(ls(pattern = "R83P3S.*")))
R83P3 <- R83P3 |>
  left_join(R83Data) |>
  mutate(urban = "R")

U83P3 <- bind_rows(mget(ls(pattern = "U83P3S.*")))
U83P3 <- U83P3 |>
  left_join(U83Data) |>
  mutate(urban = "U")
...
attr(EXP83$Table, "label") <- "Table in Part 3"
attr(EXP83$Global, "label") <- "global item code"
attr(EXP83$code, "label") <- "item code in this year"
attr(EXP83$urban, "label") <- "rural or urban"
attr(EXP83$Price, "label") <- "median price"
attr(EXP83$Value, "label") <- "monthly expenditure"
attr(EXP83$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP83, "./exported/EXP83.Rds")

###############################################
# Building individual level data

r83data <- R83Data |> select(Address, weight, province)
u83data <- U83Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R83P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R83P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R83P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R83P4S02_unique <- as.data.frame(DT) 

DT <- data.table(R83P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R83P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U83P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
            'ISCO_w'= first(ISCO_w),
            'ISIC_w'= first(ISIC_w),
            'status_w'= first(status_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U83P4S01_unique <- as.data.frame(DT) 

DT <- data.table(U83P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U83P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U83P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U83P4S03_unique <- as.data.frame(DT) 

Rind83 <- r83data  |>
  left_join(select(R83P1,-degree)) |>
  left_join(R83P4S01_unique) |>
  left_join(R83P4S02_unique) |>
  left_join(R83P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind83 <- u83data |>
  left_join(select(U83P1,-degree)) |>
  left_join(U83P4S01_unique) |>
  left_join(U83P4S02_unique) |>
  left_join(U83P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND83 <- bind_rows(Rind83,Uind83) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND83$occupationalst, "label") <- "employment status"
attr(IND83$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND83$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND83$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND83$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND83$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND83$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND83$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND83$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND83$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND83$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND83$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND83,"./exported/IND83.Rds")
...
attr(HH83$size, "label") <- "Household size"
attr(HH83$literates, "label") <- "Number of literate members"
attr(HH83$students, "label") <- "Number of student members"
attr(HH83$employeds, "label") <- "Number of employed members"
attr(HH83$gender, "label") <- "Head's gender"
attr(HH83$age, "label") <- "Head's age"
attr(HH83$literacy, "label") <- "Head's literacy"
attr(HH83$education, "label") <- "Head's education"
attr(HH83$occupationalst, "label") <- "Head's job status"
attr(HH83$maritalst, "label") <- "Head's marital status"
attr(HH83$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH83$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH83$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH83$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH83$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH83$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH83$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH83$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH83$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH83$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH83$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH83$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH83$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH83$income, "label") <- "total household income in previous year"
attr(HH83$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH83, file = "./exported/HH83.Rds")
...
df <- readRDS("./exported/HH83.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH83.dta") # to export in STATA
#write_csv2(df, "./exported/HH83.csv") # to export in CSV
