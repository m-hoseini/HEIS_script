...
R94_weight<-Sum_R94%>%select(ADDRESS, weight)%>%rename(Address = ADDRESS)%>%mutate_if(is.character, as.numeric)
R94Data <- R94Data |> 
  ...
  left_join(R94_weight)


U94_weight<-Sum_U94%>%select(ADDRESS, weight)%>%rename(Address = ADDRESS)%>%mutate_if(is.character, as.numeric)
U94Data <- U94Data |> 
  rename(month = MahMorajeh, khanevartype = NoeKhn) |>
  mutate(province = fct_recode(as.factor(substr(Address, 2, 3)), !!!Province),
         town = as.integer(substr(Address, 4, 5)))%>%
  left_join(U94_weight)
...
# Part 3, Table 13
R94P3S13$DYCOL05 <- as.numeric(levels(R94P3S13$DYCOL05))[R94P3S13$DYCOL05]
R94P3S13 <- R94P3S13 |>
  rename(
    code = DYCOL01,
...
U94P3S13$DYCOL05 <- as.numeric(levels(U94P3S13$DYCOL05))[U94P3S13$DYCOL05]
U94P3S13 <- U94P3S13 |>
  rename(
    code = DYCOL01,
    insured_loan = DYCOL02,
...
U94P4S02$DYCOL15<- as.numeric(levels(U94P4S02$DYCOL15))[U94P4S02$DYCOL15]
...
rm(list = setdiff(ls(), ls(pattern = "94"))) # removing unnecessary objects

save.image(file="./exported/HEIS94.Rdata")
...
R94P3 <- bind_rows(mget(ls(pattern = "R94P3S.*")))
R94P3 <- R94P3 |> 
  left_join(R94Data) |>
  mutate(urban = "R")

U94P3 <- bind_rows(mget(ls(pattern = "U94P3S.*")))
U94P3 <- U94P3 |> 
  left_join(U94Data) |>
  mutate(urban = "U")
...
attr(EXP94$Global, "label") <- "global item code"
attr(EXP94$code, "label") <- "item code in this year"
attr(EXP94$urban, "label") <- "rural or urban"
attr(EXP94$Price, "label") <- "median price"
attr(EXP94$Value, "label") <- "monthly expenditure"
attr(EXP94$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP94, "./exported/EXP94.Rds")

###############################################
# Building individual level data

r94data<- R94Data |> select(Address, month, weight, khanevartype, Jaygozin, province)
u94data<- U94Data |> select(Address, month, weight, khanevartype, Jaygozin, province)


# faster code with data.table
DT <- data.table(R94P4S01)
DT <- DT[,.('employed_w'= first(employed_w),
         ...
         by=.(Address, member)]
R94P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(R94P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
         ...
         by=.(Address, member)]
R94P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(R94P4S03)
...
DT <- DT[,.('employed_w'= first(employed_w),
            ...
            'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
         by=.(Address, member)]
U94P4S01_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_w,days_w), ~ifelse(.x == 0,NA,.x)))


DT <- data.table(U94P4S02)
DT <- DT[,.('employed_s'= first(employed_s),
            'ISCO_s'= first(ISCO_s),
            'ISIC_s'= first(ISIC_s),
            'status_s'= first(status_s),
            ...
            'sale'= sum(sale, na.rm = T)),
         by=.(Address, member)]
U94P4S02_unique <- as.data.frame(DT) |>
  mutate(across(c(hours_s,days_s), ~ifelse(.x == 0,NA,.x)))

DT <- data.table(U94P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            ...
            'income_transfer'= sum(income_transfer, na.rm = T)),
         by=.(Address, member)]
U94P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U94P4S04)
DT <- DT[,.('subsidy_number'= sum(subsidy_number, na.rm = T),
            'subsidy_month'= sum(subsidy_month, na.rm = T),
            'subsidy'= sum(subsidy, na.rm = T)),
         by=.(Address, member)]
U94P4S04_unique <- as.data.frame(DT) 


Rind94 <- r94data  |>
  left_join(select(R94P1,-degree)) |>
  left_join(R94P4S01_unique) |>
  left_join(R94P4S02_unique) |>
  left_join(R94P4S03_unique) |> 
  left_join(R94P4S04_unique) |>
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind94 <- u94data |>
  left_join(select(U94P1,-degree)) |>
  left_join(U94P4S01_unique) |>
  left_join(U94P4S02_unique) |>
  left_join(U94P4S03_unique) |> 
  left_join(U94P4S04_unique) |>
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")


IND94 <- bind_rows(Rind94,Uind94)%>%
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND94$occupationalst, "label") <- "employment status"
attr(IND94$employed_w, "label") <- "Whether employed in wage-earning job?"
attr(IND94$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND94$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND94$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND94$hours_w, "label") <- "Wage-earning job: hours per day"
attr(IND94$days_w, "label") <- "Wage-earning job: day per week"
attr(IND94$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND94$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND94$employed_s, "label") <- "Whether employed in non-wage job?"
attr(IND94$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND94$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND94$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND94$hours_s, "label") <- "non-wage job: hours per day"
attr(IND94$days_s, "label") <- "non-wage job: day per week"
attr(IND94$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND94,"./exported/IND94.Rds")
...
attr(HH94$size, "label") <- "Household size"
attr(HH94$literates, "label") <- "Number of literate members"
attr(HH94$students, "label") <- "Number of student members"
attr(HH94$employeds, "label") <- "Number of employed members"
attr(HH94$gender, "label") <- "Head's gender"
attr(HH94$age, "label") <- "Head's age"
attr(HH94$literacy, "label") <- "Head's literacy"
attr(HH94$education, "label") <- "Head's education"
attr(HH94$occupationalst, "label") <- "Head's job status"
attr(HH94$maritalst, "label") <- "Head's marital status"
attr(HH94$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH94$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH94$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH94$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH94$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
...
attr(HH94$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH94$income, "label") <- "total household income in previous year"
attr(HH94$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH94, file = "./exported/HH94.Rds")
...
df <- readRDS("./exported/HH94.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH94.dta") # to export in STATA
#write_csv2(df, "./exported/HH94.csv") # to export in CSV
