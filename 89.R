...
U89P3S14 <- U89P3S14%>%
  ...
  mutate(code = case_when(
          code == 33226 ~ 33236L,
          TRUE ~ code), #some wrong item codes
         cost = replace_na(as.numeric(as.character(cost)),0),
         sell = replace_na(as.numeric(as.character(sell)),0),
         value = cost - sell,
         table = 14L) |> 
...
attr(EXP89$Table, "label") <- "Table in Part 3"
attr(EXP89$Global, "label") <- "global item code"
...
attr(IND89$occupationalst, "label") <- "employment status"
...
attr(HH89$education, "label") <- "Head's education"
...
attr(HH89$income_nm_homemade, "label") <- "Non-monetary income from home production"
