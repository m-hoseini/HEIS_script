...
R72P4S1$COL02 <- as.numeric(as.character(R72P4S1$COL02))
R72P4S1$COL03 <- as.numeric(as.character(R72P4S1$COL03))
R72P4S01 <- R72P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U72P4S1$COL02 <- as.numeric(as.character(U72P4S1$COL02))
U72P4S1$COL03 <- as.numeric(as.character(U72P4S1$COL03))
U72P4S01 <- U72P4S1 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
# Part 4, Table 2
R72P4S2$COL02 <- as.numeric(as.character(R72P4S2$COL02))
R72P4S2$COL03 <- as.numeric(as.character(R72P4S2$COL03))
R72P4S02 <- R72P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
U72P4S2$COL02 <- as.numeric(as.character(U72P4S2$COL02))
U72P4S2$COL03 <- as.numeric(as.character(U72P4S2$COL03))
U72P4S02 <- U72P4S2 |>
  rename(
    Address = ADDRESS,
    member = COL01,
...
rm(R72P4S1,U72P4S1,R72P4S2,U72P4S2,R72P4S3,U72P4S3)

rm(list = setdiff(ls(), ls(pattern = "72"))) # removing unnecessary objects

save.image(file="./exported/HEIS72.Rdata")
...
R72P3 <- bind_rows(mget(ls(pattern = "R72P3S.*")))
R72P3 <- R72P3 |>
  left_join(R72Data) |>
  mutate(urban = "R")

U72P3 <- bind_rows(mget(ls(pattern = "U72P3S.*")))
U72P3 <- U72P3 |>
  left_join(U72Data) |>
  mutate(urban = "U")
...
attr(EXP72$Table, "label") <- "Table in Part 3"
attr(EXP72$Global, "label") <- "global item code"
attr(EXP72$code, "label") <- "item code in this year"
attr(EXP72$urban, "label") <- "rural or urban"
attr(EXP72$Price, "label") <- "median price"
attr(EXP72$Value, "label") <- "monthly expenditure"
attr(EXP72$Value_r, "label") <- "monthly expenditure in 1390 price"

saveRDS(EXP72, "./exported/EXP72.Rds")


###############################################
# Building individual level data

r72data <- R72Data |> select(Address, weight, province)
u72data <- U72Data |> select(Address, weight, province)

# faster code with data.table
DT <- data.table(R72P4S01)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  'ISIC_w'= first(ISIC_w),
  ...
  by=.(Address, member)]
R72P4S01_unique <- as.data.frame(DT) 

DT <- data.table(R72P4S02)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  'ISIC_s'= first(ISIC_s),
  ...
  by=.(Address, member)]
R72P4S02_unique <- as.data.frame(DT)

DT <- data.table(R72P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
         ...
         by=.(Address, member)]
R72P4S03_unique <- as.data.frame(DT) 

DT <- data.table(U72P4S01)
DT <- DT[,.(
  'ISCO_w'= first(ISCO_w),
  'ISIC_w'= first(ISIC_w),
  'status_w'= first(status_w),
  ...
  'netincome_w_y'= sum(netincome_w_y, na.rm = T)),
  by=.(Address, member)]
U72P4S01_unique <- as.data.frame(DT)

DT <- data.table(U72P4S02)
DT <- DT[,.(
  'ISCO_s'= first(ISCO_s),
  'ISIC_s'= first(ISIC_s),
  'status_s'= first(status_s),
  ...
  'sale'= sum(sale, na.rm = T)),
  by=.(Address, member)]
U72P4S02_unique <- as.data.frame(DT) 

DT <- data.table(U72P4S03)
DT <- DT[,.('income_pension'= sum(income_pension, na.rm = T),
            'income_rent'= sum(income_rent, na.rm = T),
            'income_interest'= sum(income_interest, na.rm = T),
            'income_aid'= sum(income_aid, na.rm = T),
            'income_resale'= sum(income_resale, na.rm = T)),
         by=.(Address, member)]
U72P4S03_unique <- as.data.frame(DT) 

Rind72 <- r72data  |>
  left_join(select(R72P1,-degree)) |>
  left_join(R72P4S01_unique) |>
  left_join(R72P4S02_unique) |>
  left_join(R72P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "R")

Uind72 <- u72data |>
  left_join(select(U72P1,-degree)) |>
  left_join(U72P4S01_unique) |>
  left_join(U72P4S02_unique) |>
  left_join(U72P4S03_unique) |> 
  mutate(across(where(is.character),as.integer)) |>
  mutate(urban = "U")

IND72 <- bind_rows(Rind72,Uind72) |>
  mutate(urban = as.factor(urban),
         Global = 1000000,
...
attr(IND72$occupationalst, "label") <- "employment status"
attr(IND72$ISIC_w, "label") <- "Industry code of wage-earning job"
attr(IND72$ISCO_w, "label") <- "Occupation code of wage-earning job"
attr(IND72$status_w, "label") <- "Wage-earning job status: 1-public 2-cooperative 3-private"
attr(IND72$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(IND72$netincome_w_y, "label") <- "wage earning job: net income previous year"
attr(IND72$ISIC_s, "label") <- "Industry code of non-wage job"
attr(IND72$ISCO_s, "label") <- "Occupation code of non-wage job"
attr(IND72$status_s, "label") <- "job status: 4-employer 2-selfemployed 3-familyworker"
attr(IND72$income_s_y, "label") <- "non-wage job: net income previous year"

saveRDS(IND72,"./exported/IND72.Rds")
...
attr(HH72$size, "label") <- "Household size"
attr(HH72$literates, "label") <- "Number of literate members"
attr(HH72$students, "label") <- "Number of student members"
attr(HH72$employeds, "label") <- "Number of employed members"
attr(HH72$gender, "label") <- "Head's gender"
attr(HH72$age, "label") <- "Head's age"
attr(HH72$literacy, "label") <- "Head's literacy"
attr(HH72$education, "label") <- "Head's education"
attr(HH72$occupationalst, "label") <- "Head's job status"
attr(HH72$maritalst, "label") <- "Head's marital status"
attr(HH72$ISIC_w, "label") <- "Head's Industry code of wage-earning job"
attr(HH72$ISCO_w, "label") <- "Head's Occupation code of wage-earning job"
attr(HH72$ISIC_s, "label") <- "Head's Industry code of non-wage job"
attr(HH72$ISCO_s, "label") <- "Head's Occupation code of non-wage job"
attr(HH72$income_nm_agriculture, "label") <- "Non-monetary income from agriculture"
attr(HH72$income_nm_nonagriculture, "label") <- "Non-monetary income from nonagriculture"
attr(HH72$income_nm_public, "label") <- "Non-monetary income from public sector job"
attr(HH72$income_nm_private, "label") <- "Non-monetary income from private sector job"
attr(HH72$income_nm_homemade, "label") <- "Non-monetary income from home production"
attr(HH72$netincome_w_m, "label") <- "wage-earning job: net income previous month"
attr(HH72$netincome_w_y, "label") <- "wage-earning job: net income previous year"
attr(HH72$income_w_y, "label") <- "wage-earning job: gross income previous year"
attr(HH72$income_s_y, "label") <- "non-wage job: net income previous year"
attr(HH72$income, "label") <- "total household income in previous year"
attr(HH72$expenditure, "label") <- "total monthly expendiuture of the household"

saveRDS(HH72, file = "./exported/HH72.Rds")
...
df <- readRDS("./exported/HH72.Rds") # Specify the Rds file to convert here

#haven::write_dta(df, "./exported/HH72.dta") # to export in STATA
#write_csv2(df, "./exported/HH72.csv") # to export in CSV
